...
public final class CopyManufactureFrom1S_test {
            ...
            while (rs.next()) {
                ...
                if (listManuf.containsKey(key)) {
                    if ( ! listManuf.get(key).compareTo(tempManuf)) {
                        ...
                        ps2 = conn2.prepareStatement("UPDATE kiyv.test.manufacture_test SET iddoc = ?, position = ?, docno = ?, " +
...
//                        ps2.executeBatch();
                    ...
                    ps2 = conn2.prepareStatement("INSERT INTO kiyv.test.manufacture_test (iddoc, position, docno, id_order, time_manuf, time21, amount, id_tmc, descr_second, " +
...
//            st2.execute("UPDATE orders SET time21 = time21 FROM invoice WHERE orders.iddoc = invoice.id_order AND orders.payment >= orders.price - 0.9 AND orders.time22 ISNULL;");
            st2.execute("UPDATE kiyv.test.orders_test SET time_manuf = kiyv.test.manufacture_test.time_manuf FROM kiyv.test.manufacture_test WHERE kiyv.test.orders_test.iddoc = kiyv.test.manufacture_test.id_order AND kiyv.test.orders_test.time_manuf ISNULL;");  //AND manufacture.time_manuf NOTNULL
            st2.execute("UPDATE kiyv.test.statuses_test SET time_21 = time21 FROM kiyv.test.manufacture_test " +
                    "WHERE kiyv.test.manufacture_test.id_order = kiyv.test.statuses_test.iddoc and kiyv.test.statuses_test.time_21 ISNULL;");
            st2.execute("UPDATE kiyv.test.statuses_test SET status_index = 21 WHERE time_21 NOTNULL and status_index < 21 ;");

            st2.execute("UPDATE kiyv.test.orders_test SET docno_manuf = kiyv.test.manufacture_test.docno FROM kiyv.test.manufacture_test WHERE kiyv.test.orders_test.iddoc = kiyv.test.manufacture_test.id_order AND kiyv.test.orders_test.docno_manuf ISNULL;");
    ...
    public static void readOldManufacture (Map<String, RecordManuf> listManuf) {
        ...
        try (Connection conn = DriverManager.getConnection(url, user, password); Statement st = conn.createStatement()) {
            ...
            ResultSet rs = st.executeQuery("select * from kiyv.test.manufacture_test ORDER BY iddoc, position;");
