...
public final class CopyInvoiceFrom1S {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (needChange) {
                    ...
                    ps2 = conn2.prepareStatement("UPDATE invoice SET  docno = ?, id_order = ?, time_invoice = ?, time22 = ?, price = ? WHERE iddoc = ?;");
                ...
                if (needInsert) {
                    ps2 = conn2.prepareStatement("INSERT INTO invoice (iddoc, docno, id_order, time_invoice, time22, price) VALUES (?, ?, ?, ?, ?, ?);");
...
//            System.out.println("start changeOrderPayment");
            ...
            for (String idOrder : numOrder) {
                ps1 = conn.prepareStatement("SELECT SUM(price), MAX(time22), MAX(docno) FROM invoice WHERE id_order = ?;");
...
//                    System.out.println("idDoc = " + idOrder +  ",  payment = " + payment);
                    ...
                    ps2 = conn.prepareStatement("UPDATE orders SET  payment = ?, docno_invoice = ? WHERE iddoc = ?;");
...
//            System.out.println("start changeOrderPayment");
            ...
            st.execute("UPDATE orders SET time22 = invoice.time22 FROM invoice WHERE orders.iddoc = invoice.id_order AND orders.payment >= orders.price - 0.9 AND orders.time22 ISNULL;");
            st.execute("UPDATE orders SET time_invoice = invoice.time_invoice FROM invoice WHERE orders.iddoc = invoice.id_order AND orders.time22 NOTNULL AND orders.time_invoice ISNULL;");
            st.execute("UPDATE statuses SET time_22 = time22 FROM orders " +
                    "WHERE orders.iddoc = statuses.iddoc and orders.time22 NOTNULL;");
            st.execute("UPDATE statuses SET status_index = 22 WHERE time_22 NOTNULL and status_index < 22 ;");
            st.execute("UPDATE orders SET docno_invoice = invoice.docno FROM invoice WHERE orders.iddoc = invoice.id_order AND orders.docno_invoice ISNULL;");
